...
;	fx2utils.c:43: USBCS |= bmDISCON | bmRENUM;
	...
	orl	a,#0x0A
	movx	@dptr,a
;	fx2utils.c:46: mdelay (250);			// FIXME why 1.5 seconds?
	mov	dptr,#0x00FA
	lcall	_mdelay
;	fx2utils.c:48: USBIRQ = 0xff;		// clear any pending USB irqs...
	mov	dptr,#_USBIRQ
	mov	a,#0xFF
	movx	@dptr,a
;	fx2utils.c:49: EPIRQ =  0xff;		//   they're from before the renumeration
	mov	dptr,#_EPIRQ
	mov	a,#0xFF
	movx	@dptr,a
;	fx2utils.c:51: EXIF &= ~bmEXIF_USBINT;
	anl	_EXIF,#0xEF
;	fx2utils.c:53: USBCS &= ~bmDISCON;		// reconnect USB
	mov	dptr,#_USBCS
	movx	a,@dptr
	anl	a,#0xF7
	movx	@dptr,a
